...
/**
 * Loads the path of the 404 animation from the 404.json file 
 * and inserts the image element into the HTML.
 */
function load404AnimationFromJSON() {
    const container = document.getElementById('animation-placeholder');
    if (!container) return; // Stop if the container is not found

    // Fetch the JSON file. We use '404.json' because it's in the same folder as main.js.
    fetch('404.json')
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            // 1. Create the new image element
            const img = document.createElement('img');

            // 2. Set the properties using the data loaded from 404.json
            img.src = data.animationPath;
            img.alt = data.altDescription;

            // 3. Apply styles for correct display (adjust maxWidth if the image is too large/small)
            img.style.display = 'block';
            img.style.margin = '20px auto';
            img.style.maxWidth = '100%';

            // 4. Insert the image into the HTML placeholder
            container.appendChild(img);
        })
        .catch(error => {
            console.error('Error loading 404 animation data:', error);
            container.innerHTML = '<p>Error: Could not load the animated dog image.</p>';
        });
}

// Execute the function when the page runs
load404AnimationFromJSON();
...
function initPhoneInput() {
    ...
    const updatePhoneValue = () => {
        // Just update the input's value with full international number
        phoneInput.value = iti.getNumber(); // e.g., +92 300 1234567
    };
